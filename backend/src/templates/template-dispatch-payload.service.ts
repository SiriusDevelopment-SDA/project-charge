...
export class TemplateDispatchPayloadService {
  ...
  templateRequiresInvoiceData(template: Templates): boolean {
    ...
    for (const c of comps) {
      ...
      if (t === 'HEADER' && String(c?.format ?? '').toUpperCase() === 'DOCUMENT') {
        return true;
      }
  ...
  async buildQueueRecipients(
    ...
    for (const row of rows) {
      const number = String(row.whatsapp ?? row.number ?? '').replace(/\D/g, '');
      ...
      if (!built) {
        skips.push({
          ...
          detail: 'Variáveis obrigatórias do template não puderam ser preenchidas.',
        });
        continue;
      }
  ...
  private normalizeComponents(raw: Templates['components']): BlueprintComponent[] {
    ...
    if (hasDocumentHeader) {
      ...
      if (pdfLink) {
        components.push({
          type: 'HEADER',
          parameters: [
            { type: 'document', document: { link: pdfLink, filename: 'fatura.pdf' } },
          ],
        });
      }
    }
  ...
  private buildOrderDetailsComponent(
    ...
    const shouldIncludePixKey =
      pixKeyCandidate.length > 0 && (isValidExplicit || inferredPixKeyType !== null);
    ...
    const orderDetails = {
      ...
      type: 'digital-goods',
      payment_type: 'br',
      payment_settings: [{ type: 'pix_dynamic_code', pix_dynamic_code: pixDynamicCode }],
      currency: 'BRL',
  ...
  private getOrderedTemplateVariableKeys(templateVariables: Templates['variables']): string[] {
  ...
  private applyOrderDetailsReferenceId(
    ...
    const cn =
      contractNumber?.trim() ||
      this.extractBodyVariableValue(components, templateVariables, 'numero_contrato');
    ...
    return components.map((component) => {
      ...
      if (componentType !== 'BUTTON' || subType !== 'ORDER_DETAILS') return component;
