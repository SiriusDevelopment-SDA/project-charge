...
export class InvoicesService {
  ...
  async getInvoicesBatch(data: SearchRequestInvoicesDto): Promise<InvoiceBatchResponseDto> {
    ...
    for (const doc of documents) {
      try {
        ...
        if (!cliente) {
          errors.push({ document: doc, reason: 'Cliente não encontrado' });
          continue;
        }
        ...
        errors.push({ document: doc, reason: 'Erro inesperado ao processar o cliente' });
      }
    }
  ...
  async searchOverdueClients(body: {
    account: string;
    query?: string;
    page?: number;
    limit?: number;
    ...
    if (!account) throw new BadRequestException('Account é obrigatório.');
    ...
    const hasStructuredFilter =
      agingMin != null || agingMax != null || debtMin != null || debtMax != null;
    ...
    // Summary counts — cached in Redis for 60s (change only on sync)
    const summaryCacheKey = `overdue-summary:${account}`;
    const [summaryRow, cachedSummaryExtra] = await Promise.all([
      baseQuery
        .clone()
    ...
    let summaryExtra = cachedSummaryExtra;
    if (!summaryExtra) {
      const [mappedClients, checkedClients, clientsWithSnapshot, clientsWithOpenInvoices] =
        await Promise.all([
          this.clientRepo
            .createQueryBuilder('client')
            .innerJoin('client.company', 'company')
      ...
      summaryExtra = { mappedClients, checkedClients, clientsWithSnapshot, clientsWithOpenInvoices };
      await this.redisService.set(summaryCacheKey, summaryExtra, 60);
    }
    ...
    if (hasStructuredFilter) {
      // Fix: use pagination + count subquery instead of loading all IDs into memory
      const countClone = groupedQuery.clone().select('1', '__dummy');
      const [innerSql, innerParams] = countClone.getQueryAndParameters();

      const [pageRows, countResult] = await Promise.all([
        groupedQuery.clone().offset(skip).limit(safeLimit).getRawMany<{ id: string }>(),
        this.clientRepo.manager.query<Array<{ cnt: string }>>(
          `SELECT COUNT(*) as cnt FROM (${innerSql}) AS __counted__`,
          innerParams,
        ),
      ]);
    ...
    if (!clientIds.length) {
      return { data: [], page: safePage, limit: safeLimit, total };
    }

    // Fix: conditional join loads only overdue invoices — avoids fetching ALL invoices per client
    const clients = await this.clientRepo
      ...
      .leftJoinAndSelect(
        'client.invoices',
        'invoice',
        `LOWER(TRIM(invoice.status)) = 'a receber' AND ${this.getInvoiceDueDateSql('invoice.expiration')} < :today`,
        { today: todaySql },
      )
    ...
    const data = clients
      .map((client) => {
        // Invoices already filtered to overdue-only by the conditional join above
        const overdueInvoices = (client.invoices ?? []).sort((a, b) => {
    ...
    return {
      ...
      summary: {
        ...
        mappedClients: summaryExtra.mappedClients,
        checkedClients: summaryExtra.checkedClients,
        clientsWithSnapshot: summaryExtra.clientsWithSnapshot,
        clientsWithOpenInvoices: summaryExtra.clientsWithOpenInvoices,
      },
    };
  }

  async getOpenClientIds(account: string): Promise<{ clientIds: string[] }> {
    const company = await this.companyRepo.findOne({
      where: { account_chatwoot: account },
    });

    if (!company) return { clientIds: [] };
  ...
  async getSyncState(account: string) {
  ...
  async syncCompanyInvoices(companyId: string) {
    ...
    return { message: 'Sincronização iniciada.', companyId, status: 'running' };
  }

  async getPixBatch(data: PixBatchRequestDto) {
  ...
  private mapInvoiceEntityToDto(inv: Invoice): InvoiceMapResultDto {
    return {
      ...
      invoice_due_date: this.toBrDate(inv.expiration),
