...
- **B3 · fix(invoices): `pix/batch` orientado a `pixStrategy`** — `ErpDefinition` ganha `pixStrategy` substituindo o `pix: boolean` sem leitores; handler ramifica por capacidade; ERP desconhecido responde erro explícito; docblock corrigido (nunca citou GAMAISP). **M** · Decisão 2 **resolvida por evidência**: a Gama e `inline` puro, sem fallback — ver a tabela de decisões.
...
| 2 | ~~`pixStrategy` da Gama: sempre ao vivo ou snapshot com fallback on-demand?~~ **RESOLVIDA (04/09) por sonda, não por recomendação** | B3 | **`inline`, sem fallback.** O fallback chegou a ser implementado (PR #102) e foi **revertido**: a sonda direta na API do Gama provou que `/api/v1/faturas/id/{id}` e `/faturas/doc/{cpf}` leem o MESMO registro — fatura sem `pix_qrcode` na listagem também vem sem por id. O fallback nunca teve como devolver valor diferente e custava 1 chamada por fatura sem PIX (7.165 de 13.201 na POWERNET). **Não reintroduzir no B3.** A causa do PIX ausente é configuração de cobrança do lado do Gama; o paliativo é template com linha digitável. |
