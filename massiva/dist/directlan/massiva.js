...
const MASSIVA_URL = WEBHOOK_BASE + "/webhook/salvadados-directlan";
const HISTORICO_URL = WEBHOOK_BASE + "/webhook/historico-directlan";
const CATALOGO_URL = WEBHOOK_BASE + "/webhook/catalogo-directlan";
const CATALOGO_CRIAR_URL = WEBHOOK_BASE + "/webhook/catalogo-criar-directlan";
const CATALOGO_EXCLUIR_URL = WEBHOOK_BASE + "/webhook/catalogo-excluir-directlan";
...
// #filial pode não existir em esqueletos antigos — guarda contra null.
const filialEl = document.getElementById("filial");
const filialBruto = filialEl ? filialEl.textContent.trim() : "";
...
// Filial dentro da MESMA conta. Ausente/placeholder => "1" (bate com o DEFAULT '1' do banco).
// Todas as empresas de filial única continuam funcionando sem enviar nada (caem na filial 1).
const filial_id = ehPlaceholder(filialBruto) ? "1" : (filialBruto || "1");
...
const Store = {
  _cache: null,
  _carregando: null,
  _invalidar() { this._cache = null; this._carregando = null; },

  // Carrega tudo de uma vez (modelos + catálogo). Memoriza até uma escrita.
  // Guarda a PROMESSA em voo: se dois pontos do init chamarem quase juntos,
  // ambos compartilham a MESMA requisição (evita ler o catálogo 2x).
  async carregarTudo(account) {
    if (this._cache) return this._cache;
    if (this._carregando) return this._carregando;
    this._carregando = Promise.resolve(
      MODO_DEV ? this._carregarLocal(account) : this._carregarRemoto(account)
    ).then((r) => { this._cache = r; return r; })
  ...
  async _criar(account, tipo, campos) {
    ...
    const r = await fetch(CATALOGO_CRIAR_URL, {
      ...
      body: JSON.stringify({
        account: account, token: token_id, tipo: tipo, filial_id: filial_id,
  ...
  async _excluir(account, id) {
    ...
    const r = await fetch(CATALOGO_EXCLUIR_URL, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ account: account, token: token_id, id: id, filial_id: filial_id }),
    });
  ...
  async _carregarRemoto(account) {
    const url = CATALOGO_URL + "?account=" + encodeURIComponent(account) + "&token=" + encodeURIComponent(token_id) + "&filial_id=" + encodeURIComponent(filial_id);
...
const CHAVE_ATIVACAO = "massiva_ativada_em_" + account_id + "_f" + filial_id;
...
async function carregarHistorico() {
  ...
  try {
    const resposta = await fetch(`${HISTORICO_URL}?account=${encodeURIComponent(account_id)}&token=${encodeURIComponent(token_id)}&filial_id=${encodeURIComponent(filial_id)}`);
